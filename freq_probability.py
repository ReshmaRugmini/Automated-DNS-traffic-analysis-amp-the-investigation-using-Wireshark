...
def char_frequency_probability(domain):  #Calculate the probability of each character in the domain.
    domain = domain.lower()
    length = len(domain)
    freq = Counter(domain)
...
def calculate_entropy(probabilities):       #Calculate the Shannon entropy of the domain based on character probabilities.
    entropy = -sum(p * math.log2(p) for p in probabilities.values())
    return entropy

def analyze_dns_signature(capture): # Analyze DNS signatures from the captured packets.
    dns_queries = []
    dns_record_types = []
